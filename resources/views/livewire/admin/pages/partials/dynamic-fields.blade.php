...
@foreach ($fields as $field)
    ...
    <div class="mt-4">
        ...
        @elseif (($field['type'] ?? 'text') === 'image')
            <div x-data="{
                preview: @js(filled($value) ? \Illuminate\Support\Facades\Storage::disk('public')->url($value) : ''),
            ...
            x-on:media-selected.window="
                if ($event.detail.fieldPath === fieldPath && String($event.detail.sectionId ?? '') === String(sectionId ?? '')) {
                    ...
                    $wire.set(wireKey, $event.detail.path);
                }
            ">
                ...
                <div class="flex items-center gap-2 mt-2">
                    ...
                    <input type="file" wire:model="{{ $uploadPrefix }}{{ $fieldPath }}" accept="image/*"
                        class="block flex-1 text-xs text-gray-500 file:mr-3 file:rounded-md file:border-0 file:bg-indigo-50 file:px-3 file:py-1.5 file:text-[10px] file:font-semibold file:text-indigo-600 hover:file:bg-indigo-100">
                </div>
            </div>
